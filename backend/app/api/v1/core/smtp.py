...
class AsyncEmailSender:
    """기본 비동기 SMTP 메일러"""
    class AsyncEmailSenderSettings(BaseModel):
        smtp_host: str
        smtp_port: int
        username: str
        password: str
    ...
    def __init__(self, setting: AsyncEmailSenderSettings):
...
async def get_smtp_client(request: Request) -> AsyncEmailSender:
    """
    FastAPI Request에서 AsyncEmailSender 가져오기
    - app.state.smtp에 이미 초기화되어 있다면 그대로 반환
    - 없으면 예외 발생 또는 초기화 처리 가능
    """
    smtp_client: AsyncEmailSender | None = getattr(request.app.state, "smtp", None)
